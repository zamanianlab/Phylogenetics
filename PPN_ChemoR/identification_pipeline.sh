...
### Build HMMs
# ChemoRs
# cat "${gh_dir}"/aux/pfam_HMMs/GPCR/ChemoR/*hmm > "${gh_dir}"/aux/pfam_HMMs/GPCR/ChemoR/ChemoR.hmm
# hmmpress "${gh_dir}"/aux/pfam_HMMs/GPCR/ChemoR/ChemoR.hmm
# ChemoR_HMM="${gh_dir}"/aux/pfam_HMMs/GPCR/ChemoR/ChemoR.hmm

## GRAFS+
# cat "${gh_dir}"/aux/pfam_HMMs/GPCR/Primary/*hmm > "${gh_dir}"/aux/pfam_HMMs/GPCR/Primary/GPCRfams.hmm
# hmmpress "${gh_dir}"/aux/pfam_HMMs/GPCR/Primary/GPCRfams.hmm
# GRAFS_HMM="${gh_dir}"/aux/pfam_HMMs/GPCR/Primary/GPCRfams.hmm

## Combined (GRAFS+ / ChemoR)
# cat "${gh_dir}"/aux/pfam_HMMs/GPCR/Primary/GPCRfams.hmm "${gh_dir}"/aux/pfam_HMMs/GPCR/ChemoR/ChemoR.hmm > "${gh_dir}"/aux/pfam_HMMs/GPCR/GRAFS_NemChR.hmm
# hmmpress "${gh_dir}"/aux/pfam_HMMs/GPCR/GRAFS_NemChR.hmm
GRAFS_ChemoR_HMM="${gh_dir}"/aux/pfam_HMMs/GPCR/GRAFS_ChemoR.hmm

## Prepare Pfam-A HMM db
# wget -nc -O $gh_dir/aux/pfam_HMMs/Pfam-A.hmm.gz ftp://ftp.ebi.ac.uk/pub/databases/Pfam/current_release/Pfam-A.hmm.gz
# gzcat $gh_dir/aux/pfam_HMMs/Pfam-A.hmm.gz > $gh_dir/aux/pfam_HMMsaux/HMMs/Pfam-A.hmm
# hmmpress $gh_dir/aux/pfam_HMMs/Pfam-A.hmm
...
#     array=($(echo "$line" | sed 's/\// /g'))
#     species=${array[0]}
#     hmmsearch --tblout $out/2/${species}_2.out --noali --cpu 4 $gh_dir/aux/pfam_HMMs/Pfam-A.hmm $out/1/${species}_1.fa
#   done;
# done <$species
...
#     cat $out/2/${species}_2.out | awk '{print $1 " " $3 " " $4 " " $5}' | awk '!/#/' | sort -k1,1 -k4,4g | sort -uk1,1 | awk '/7tm_1|7TM_GPCR_S|Srg|Sre|Serpentine_r_xa/' | sort -k4 -g  | awk '{print $1}' > $out/2/${species}_2_ids.txt
    ...
    # grep -v -f $out/2/${species}_2_ids.txt $out/1/${species}_1_ids.txt > $out/2/${species}_2_filtered.txt
#   done;
# done <$species
...
#     cd $db/caenorhabditis_elegans/PRJNA13758/
#     blastp -query $out/2/${species}_2.fa -db caenorhabditis_elegans.PRJNA13758.WBPS14.protein.fa -out $out/3/${species}_3.out -num_threads 4 -evalue 0.01 -outfmt '6 qseqid sseqid pident ppos length mismatch evalue bitscore'
#   done;
# done <$species
...
while IFS= read -r line; do
  for f in $db/$line/*.protein.fa.gz ; do
    ...
    cat $out/3/${species}_3.out | awk '{print $1}' | grep -wFv -f -  $out/2/${species}_2_ids.txt > $out/3/${species}_3_nohits.txt
    ## Find genes that had a significant hit to a C. elegans ChemoR
    cat $out/3/${species}_3.out | awk '{print $1 " " $2 " " $7}' | sort -k1,1 -k3,3g | sort -uk1,1 | grep -wF -f "${gh_dir}"/aux/ChemoR/simplemine_results.txt | sort -k3 -g > $out/3/${species}_3.txt
    ...
    rm $curr_dir/protein.tmp.fa
    grep -v -f $out/3/${species}_3_ids.txt $out/2/${species}_2_ids.txt > $out/3/${species}_3_filtered.txt
  done;
done <$species
