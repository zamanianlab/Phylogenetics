...
seqtk subseq $proteomes/caenorhabditis_elegans.PRJNA13758.WBPS17.protein.fa $Ce_seeds/Ce_BARs.list.txt > $Ce_seeds/Ce_seed."$line_sub".fasta
...
while IFS= read -r paradb; do
    ...
    blastp -query $Para_targets/"$line_sub"."$para_name".fasta -db $proteomes/caenorhabditis_elegans.PRJNA13758.WBPS17.protein.fa -out $Para_recip/"$line_sub"."$para_name".out -outfmt "6 qseqid sseqid pident evalue qcovs" -max_hsps 1 -evalue 1E-3 -num_threads $threads
